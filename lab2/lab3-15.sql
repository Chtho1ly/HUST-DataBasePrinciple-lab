...
SELECT TOP 1 close_contact.case_p_id, person.fullname, COUNT(close_contact.case_p_id) AS infected_number
FROM person JOIN close_contact ON person.id = close_contact.case_p_id
GROUP BY person.id, person.fullname, close_contact.case_p_id
ORDER BY COUNT(close_contact.case_p_id) DESC;
